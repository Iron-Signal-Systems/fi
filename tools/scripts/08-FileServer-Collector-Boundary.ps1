...
$ExpectedCollectorBinPath = '"C:\Program Files\FI\fi.exe" -service -service-collection-every 15s -service-supporting-refresh-every 45s'
...
if ($OriginalCollectorBinPath -ne $ExpectedCollectorBinPath) {
    ...
    throw "FICollector binary path does not match the validated preflight value. No changes made."
}

Write-Host ""
Write-Host "FICollector exact service-token boundary probe"
