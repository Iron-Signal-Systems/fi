...
## Post-Gate-1 250K randomized nested onboarding campaign — planned

The next scale campaign is additional characterization. It does not replace or
rewrite the accepted 100K Gate 1 result above.

Planned dataset characteristics:

- 250,000 files;
- randomized, reproducible tree generation with the seed recorded in the result;
- irregular directory fan-out rather than fixed 100-file buckets;
- mixed shallow, medium, and deep branches;
- maximum directory depth of 15 below the governed root;
- a mixture of empty directories, lightly populated branches, and dense leaves;
- randomized file sizes and names with the exact generated byte total recorded
  before FI starts; and
- path/name generation bounded to valid Windows/NTFS limits.

The dataset is created with FI stopped. FI then starts from a clean onboarding
state so the initial baseline owns checkpoint creation and anchored catch-up.

The full Phase 2 sender/receiver path remains active during onboarding. The
campaign records collector/helper CPU and RAM, host CPU/memory, Y: logical
I/O/queue observations, spool/generation backlog, generation compression and
cadence, sender retirement, receiver custody, and end-to-end completion.

A controlled rename/content mutation is made during the initial baseline. The
independent service USN worker is expected to skip the root until the initial
checkpoint exists; the baseline's anchored catch-up must preserve the mutation.
After a continuous checkpoint exists, the independent USN lane is validated
again under ongoing configured work.

The campaign is not complete until the exact dataset shape, seed, generated byte
count, executable hashes, start/end times, source checkpoint transitions,
generation/receipt counts, and receiver custody results are recorded.

See `PHASE-2-250K-RANDOM-NESTED-ONBOARDING-PLAN.md`.
