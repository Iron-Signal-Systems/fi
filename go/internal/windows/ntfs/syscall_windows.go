...
// Compile-time ABI checks for fixed Windows structures passed to Kernel32.
var (
...
func queryFileInformation(handle syscall.Handle, class uint32, output unsafe.Pointer, size uintptr) error {
	r1, _, callErr := procGetFileInformationByHandleEx.Call(
		uintptr(handle), uintptr(class), uintptr(output), size,
	)
	if r1 == 0 {
		return callErr
	}
...
func queryNativeState(handle syscall.Handle) (nativeState, error) {
	...
	if err := queryFileInformation(handle, fileBasicInfoClass, unsafe.Pointer(&state.Basic), unsafe.Sizeof(state.Basic)); err != nil {
		return nativeState{}, &Error{Stage: StageMetadata, Op: "GetFileInformationByHandleEx(FileBasicInfo)", Err: err}
	}
	if err := queryFileInformation(handle, fileStandardInfoClass, unsafe.Pointer(&state.Standard), unsafe.Sizeof(state.Standard)); err != nil {
		return nativeState{}, &Error{Stage: StageMetadata, Op: "GetFileInformationByHandleEx(FileStandardInfo)", Err: err}
	}
	if err := queryFileInformation(handle, fileIdInfoClass, unsafe.Pointer(&state.ID), unsafe.Sizeof(state.ID)); err != nil {
		return nativeState{}, &Error{Stage: StageIdentity, Op: "GetFileInformationByHandleEx(FileIdInfo)", Err: err}
	}
	return state, nil
}
...
func queryStreams(handle syscall.Handle) ([]nativeStream, error) {
	for size := initialStreamInfoBuffer; size <= maximumStreamInfoBuffer; size *= 2 {
		...
		return nil, &Error{Stage: StageStreams, Op: "GetFileInformationByHandleEx(FileStreamInfo)", Err: callErr}
	}
	return nil, &Error{Stage: StageStreams, Op: "GetFileInformationByHandleEx(FileStreamInfo)", Err: ErrStreamBufferLimit}
}
...
func parseStreamInfo(buffer []byte) ([]nativeStream, error) {
	...
	for offset := 0; ; {
		...
		if next < fileStreamInfoHeader || next%8 != 0 || offset+next <= offset || offset+next > len(buffer) {
			return nil, ErrMalformedStreamInfo
		}
		offset += next
	}
...
func queryVolume(handle syscall.Handle) (string, error) {
	...
	r1, _, callErr := procGetVolumeInformationByHandleW.Call(
		...
		uintptr(unsafe.Pointer(&fileSystemBuffer[0])), uintptr(len(fileSystemBuffer)),
	)
	if r1 == 0 {
		return "", callErr
	}
...
func finalVolumePath(handle syscall.Handle) ([]uint16, error) {
	...
	for attempts := 0; attempts < 3; attempts++ {
		length, _, callErr := procGetFinalPathNameByHandleW.Call(
			uintptr(handle), uintptr(unsafe.Pointer(&buffer[0])), uintptr(len(buffer)),
