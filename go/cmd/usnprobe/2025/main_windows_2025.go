...
const (
	probeVersion = "windows-server-2025-usn-characterization/0.2"
	serviceName  = "FIUSNProbe2025"

	probeInputPath  = `C:\FI-Test\usnprobe\input-2025.json`
	probeResultPath = `C:\FI-Test\usnprobe\usn-access-matrix-2025.json`
	probeErrorPath  = `C:\FI-Test\usnprobe\usn-access-matrix-2025-error.txt`
...
func loadProbeInput() (probeInput, error) {
	...
	if value.JournalID == "" {
		return probeInput{}, errors.New("input journal_id is empty")
	}

	if value.StartUSN == "" {
		return probeInput{}, errors.New("input start_usn is empty")
	}
...
func runAccessCase(
	...
	buffer, returned, err := readJournal(handle, startUSN, journalID)
	if err != nil {
...
func runProbe() probeResult {
	...
	result.SeManageVolumePrivilege = enablePrivilege("SeManageVolumePrivilege")
	...
	tests := []accessCase{
		...
		{Name: "FileReadData", Mask: fileReadData},
		{Name: "FileReadDataAttributes", Mask: fileReadData | fileReadAttributes},
		{
		...
		{Name: "GenericRead", Mask: genericRead},
	}

	for _, test := range tests {
		result.Results = append(
			result.Results,
			runAccessCase(result.Volume, test, startUSN, journalID),
		)
	}
...
func summarizeReadBuffer(buffer []byte) (string, int, error) {
	if len(buffer) < 8 {
		return "", 0, errors.New("USN buffer is shorter than continuation USN")
	}

	continuation := int64(binary.LittleEndian.Uint64(buffer[:8]))
	offset := 8
	recordCount := 0
	...
	for offset < len(buffer) {
		...
		if recordLength < usnV2HeaderSize {
			return "", 0, fmt.Errorf("invalid USN record length %d", recordLength)
		}
